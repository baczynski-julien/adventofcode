package com.jbi.advent2020.day11
...
object Day11 : DailySolution() {
    ...
    override fun part1(reader: BufferedReader): Any {
        ...
        var newLayout = applyRules(layout, ::applyEmptyRule, ::applyOccupiedRule)
        while (newLayout.second != previouslyOccupied) {
            previouslyOccupied = newLayout.second
            newLayout = applyRules(newLayout.first, ::applyEmptyRule, ::applyOccupiedRule)
        }
    ...
    override fun part2(reader: BufferedReader): Any {
        var previouslyOccupied = 0
        var newLayout = applyRules(layout, ::applyEmptyRule2, ::applyOccupiedRule2)
        while (newLayout.second != previouslyOccupied) {
            previouslyOccupied = newLayout.second
            newLayout = applyRules(newLayout.first, ::applyEmptyRule2, ::applyOccupiedRule2)
        }
        return newLayout.second
    }
    ...
    private fun logMatrix(matrix: Array<Array<Slot>>) =
        matrix.forEach { log(it.fold(StringBuilder()) { b, c -> b.append(c) }.toString()) }
